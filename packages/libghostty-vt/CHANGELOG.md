...
- Republish for tag-alignment with `blinkyterm@0.1.2` (the v0.6.2
  release succeeded for libghostty-vt but the blinkyterm publish
  step failed in the same run, so the tag/version pair couldn't be
  reused). No library code changes; functionally identical to 0.6.0.

## [0.6.2] - 2026-05-07

### Changed

- Republish to support correctly-resolved `blinkyterm@0.1.2`. No
  library code changes; functionally identical to `0.6.0` and `0.6.1`.

## [0.6.1] - 2026-05-07

### Changed

- Re-publish to align with the addition of `blinkyterm` to the
  release pipeline. No library code changes; `0.6.1` is functionally
  identical to `0.6.0`.
